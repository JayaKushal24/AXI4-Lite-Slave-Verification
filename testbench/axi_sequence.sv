...
class axi_random_sequence extends axi_base_sequence;
	...
	int aw_count,w_count,b_count,ar_count,r_count;
	task body();
		...
		repeat(100) begin
			...
			finish_item(req);
			get_response(rsp);
			if((rsp.AWVALID&&rsp.AWREADY)||aw_count==2)	begin	
				...
				aw_count=0;	end
			else if(!rsp.AWVALID)	aw_count++;
			else  aw_count=0;
			
			if((rsp.WVALID&&rsp.WREADY)||w_count==2)	begin	
				assert(req.randomize(WDATA,WSTRB,WVALID)with {WVALID dist {1:=80,0:=20};});
				w_count=0;
			end
			else if(!rsp.WVALID)	w_count++;
			else w_count=0;
			
			if((rsp.BVALID&&rsp.BREADY)||b_count==2)	begin	
				assert(req.randomize(BREADY)with {BREADY dist{1:=80,0:=20};});
				b_count=0;
			end
			else if(!rsp.BREADY)		b_count++;
			else b_count=0;
			
			if((rsp.ARVALID&&rsp.ARREADY)||ar_count==2)	begin	
				assert(req.randomize(ARADDR,ARPROT,ARVALID)with {
				ARVALID dist{1:=80,0:=20};
				ARADDR dist{[8'h00:8'h27]:/40,[8'h28:8'h33]:/40,8'h3C:/5,[8'h34:8'h3B]:/10,[8'h40:8'hFF]:/15};
				ARADDR[1:0] dist{2'b00:=80,[2'b01:2'b11]:=20};
				});
				ar_count=0;
			end
			else if(!rsp.ARVALID)	ar_count++;
			else ar_count=0;
			
			if((rsp.RVALID&&rsp.RREADY)||(r_count==2))	begin	
				assert(req.randomize(RREADY) with {RREADY dist{1:=80,0:=20};});
				r_count=0;
			end
			else if(!rsp.RREADY)	r_count++;
			else r_count=0;
